...
/* lexical rules */
<YYINITIAL> {
    "_"        { return new XiToken(TokenType.UNDERSCORE,
        yyline, yycolumn, yytext()); }

    /* keywords */
    "use"       { return new XiToken(TokenType.USE,
        yyline, yycolumn, yytext()); }
    "if"        { return new XiToken(TokenType.IF,
        yyline, yycolumn, yytext()); }
    "while"     { return new XiToken(TokenType.WHILE,
        yyline, yycolumn, yytext()); }
    "else"      { return new XiToken(TokenType.ELSE,
        yyline, yycolumn, yytext()); }
    "return"    { return new XiToken(TokenType.RETURN,
        yyline, yycolumn, yytext()); }
    "length"    { return new XiToken(TokenType.LENGTH,
        yyline, yycolumn, yytext()); }
    "int"       { return new XiToken(TokenType.INT_TYPE,
        yyline, yycolumn, yytext()); }
    "bool"      { return new XiToken(TokenType.BOOL_TYPE,
        yyline, yycolumn, yytext()); }
    "true"      { return new XiToken(TokenType.BOOL_LIT,
        yyline, yycolumn, true); }
    "false"     { return new XiToken(TokenType.BOOL_LIT,
        yyline, yycolumn, false); }

    /* identifiers */
    {ID}        { return new XiToken(TokenType.ID,
        yyline, yycolumn, yytext()); }

    /* literals */
    {INTEGER}   {
        try {
            return new XiToken(TokenType.INT_LIT,
                yyline, yycolumn, Long.parseLong(yytext()));
        } catch (NumberFormatException e) {
            return new XiToken(TokenType.ERROR,
                yyline, yycolumn, "invalid integer");
        }
    }
    ...
    "*>>"   { return new XiToken(TokenType.HI_MULT,
        yyline, yycolumn, yytext()); }
    ...
    ";"  { return new XiToken(TokenType.SEMICOLON,
        yyline, yycolumn, yytext()); }
    ...
    {WHITESPACE} {} // Ignore
    {COMMENT} {} // Ignore
    "-9223372036854775808"  { return new XiToken(TokenType.INT_LIT,
        yyline, yycolumn, Long.MIN_VALUE); }    // TODO: do we really need this?

}

<STRING> {
    \"              { yybegin(YYINITIAL);
                      return new XiToken(TokenType.STRING_LIT,
        yyline, stringLiteralStartCol, stringLiteral.toString()); }
    {EOL}           { yybegin(YYINITIAL); return new XiToken(TokenType.ERROR,
        yyline, stringLiteralStartCol, "missing ending double quotes"); }
    ...
    \\.             { yybegin(YYINITIAL); return new XiToken(TokenType.ERROR,
        yyline, yycolumn, "invalid escape character"); }
}
...
/* error fallback */
[^] { return new XiToken(TokenType.ERROR,
    yyline, yycolumn, "illegal symbol <" + yytext() + ">"); }
